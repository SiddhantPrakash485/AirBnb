...
function Card({
  ...
  return (
    <div
      ...
      onClick={() => {
        !isBooked ? handleClick() : null;
      }}
    >
      {isBooked && (
        <div className="text-[green] bg-white rounded-lg absolute flex items-center justify-center right-1 top-1 gap-[5px] p-[5px] text-[23px] font-semibold">
      ...
      {isBooked && host == userData?._id && (
        <div
          className="text-[red] bg-white rounded-lg absolute flex items-center justify-center right-1 top-[50px] gap-[5px] p-[5px] text-[23px] font-semibold"
        ...
        <div className="w-[300px] h-[100px] bg-[#ffffffdf] absolute top-[110px] left-[13px] rounded-lg">
          ...
          <div className="w-[100%] h-[50%] text-[18px] font-semibold flex items-start justify-center gap-[10px] text-[#986b6b] ">
            ...
            <button className="px-[20px] bg-[red] text-[white] rounded-lg hover:bg-slate-600" onClick={()=>{
              cancelBooking(id);setPopUp(prev=>!prev)
            }}>
              Yes
            </button>
            <button
